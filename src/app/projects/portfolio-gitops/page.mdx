...
| Domain                     | Application                                   |
| -------------------------- | --------------------------------------------- |
| `danycb.tech`              | Personal portfolio website (Next.js / SSR)    |
| `doc-analyzer.danycb.tech` | FinDoc — an LLM-powered RAG document analyzer |
...
flowchart LR
    ...
    Gateway -->|"websecure:443\nwildcard TLS"| Portfolio["danycb.tech / www.danycb.tech\n→ portfolio:80"]
    Gateway -->|"websecure:443\nwildcard TLS"| DocFrontend["doc-analyzer.danycb.tech\n→ frontend:80"]
```

A single wildcard certificate (`*.danycb.tech` + apex `danycb.tech`) is issued via Let's Encrypt ACME DNS-01 through Cloudflare's API, stored in the `traefik` namespace, and shared across all current and future subdomains. No HTTP-01 endpoint exposure required.
...
flowchart TD
    ...
    LetsEncrypt["Let's Encrypt\nACME DNS-01"] -->|"cert-manager\nauto-renew"| TLS["wildcard-danycb-tech-tls\nTraefik namespace"]
```
